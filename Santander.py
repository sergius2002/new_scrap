...
console_handler.setLevel(logging.INFO)  # Cambiado de DEBUG a INFO
...
def calcular_hash(row):
    ...
    # logger.debug(f"Calculando hash para: {data}")  # Comentado para reducir verbosidad
    ...
    for archivo in os.listdir(CARPETA_ARCHIVOS):
        ...
        if cadena_encontrada:
            ...
            try:
                ...
                # logger.info(f"\nContenido del archivo {archivo}:")  # Comentado para reducir verbosidad
                # logger.info("\nPrimeras 5 filas del archivo original:")  # Comentado para reducir verbosidad
                # logger.info(df.head().to_string())  # Comentado para reducir verbosidad
                
                columnas_relevantes = df[["MONTO", "DESCRIPCIÓN MOVIMIENTO", "FECHA"]].copy()
                
                # logger.info("\nColumnas relevantes seleccionadas:")  # Comentado para reducir verbosidad
                # logger.info(columnas_relevantes.head().to_string())  # Comentado para reducir verbosidad
                
                # Después de procesar los datos
                # logger.info("\nDatos procesados (después de normalización):")  # Comentado para reducir verbosidad
                # logger.info(columnas_relevantes.head().to_string())  # Comentado para reducir verbosidad
                ...
                def extraer_rut(descripcion):
                    ...
                    # logger.info(f"Buscando RUT en descripción: {descripcion_limpia}")  # Comentado para reducir verbosidad
                    matches = rut_regex.findall(descripcion_limpia)
                    if matches:
                        numero, verificador = matches[0]
                        rut_completo = f"{numero}-{verificador.upper()}"
                        # logger.info(f"RUT encontrado: {rut_completo}")  # Comentado para reducir verbosidad
                        return rut_completo
                    else:
                ...
                # logger.info(f"Primeras fechas originales en {archivo}:")  # Comentado para reducir verbosidad
                # logger.info(columnas_relevantes["FECHA"].head().to_string())  # Comentado para reducir verbosidad
                ...
                # logger.info(f"Primeras fechas procesadas en {archivo}:")  # Comentado para reducir verbosidad
                # logger.info(columnas_relevantes["fecha"].head().to_string())  # Comentado para reducir verbosidad
                ...
                # Después de crear el hash, cambiar el nombre de la empresa agregando "SAN CRISTOBAL SANTANDER" + últimos 3 dígitos de la cadena
                empresa_original = MAPEO_EMPRESAS.get(cadena_encontrada, "Desconocida")
                ultimos_tres_digitos = cadena_encontrada[-3:]  # Obtener los últimos 3 dígitos de la cadena
                empresa_nueva = f"SAN CRISTOBAL SANTANDER {ultimos_tres_digitos}"
                columnas_relevantes["empresa"] = empresa_nueva
                logger.info(f"Nombre de empresa actualizado de '{empresa_original}' a '{empresa_nueva}'")
...
def BASE_DE_DATOS(df_resultado, supabase_client):
    ...
    # logger.info("Primeras filas a insertar en la base de datos:")  # Comentado para reducir verbosidad
    # logger.info(df_resultado.head().to_string())  # Comentado para reducir verbosidad
    ...
    # logger.debug(f"Hashes en DataFrame a insertar: {df_resultado['hash'].tolist()}")  # Comentado para reducir verbosidad
    ...
    # for idx, row in df_nuevos.iterrows():  # Comentado para reducir verbosidad
    #     data_str = f"{row['monto']}_{row['fecha']}_{row['rut']}"  # Comentado para reducir verbosidad
    #     logger.debug(f"Registro a insertar: {data_str} -> Hash: {row['hash']}")  # Comentado para reducir verbosidad
    ...
    if datos_insertar:
        # logger.info("Primeras filas con 'rs' asignado a insertar:")  # Comentado para reducir verbosidad
        # logger.info(pd.DataFrame(datos_insertar).head().to_string())  # Comentado para reducir verbosidad
